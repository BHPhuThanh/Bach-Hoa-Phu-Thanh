...
export default function App({ standaloneInboundCreate = false } = {}) {
  ...
  const showPosPersistErrorToast = useCallback((text) => {
    const t = catalogDbErrorToastMessage(text)
    if (!t) return
  ...
  const handleUpdateCatalogVariant = useCallback(async (variantId, patch, editingOriginalCode) => {
      ...
      if (stockTouched && affectedIdsForLog.length) {
        ...
        if (any && isSupabaseConfigured()) {
          inventoryLogMeta = { prev, next, ids: affectedIdsForLog }
        }
      }
   ...
   */
  const handleBulkPatchCatalogVariants = useCallback((patches, opts) => {
    const valid = (patches || []).filter(
      (e) => e && e.variantId != null && e.patch && typeof e.patch === 'object'
    )
    const updatedCount = valid.length
    if (updatedCount === 0) return Promise.resolve({ ok: true, updatedCount: 0 })
  ...
  useEffect(() => {
    ...
    if (sellOrders.some((o) => o.id === activeSellOrderId)) return
  ...
  const handleThanhToan = useCallback(async () => {
    ...
    const snapshotCustPhone = String(checkoutOrder.customerPhone ?? '').trim()
    const snapshotNoteStr = String(checkoutOrder.orderNote ?? '').trim()
    const hasInvalidQty = snapshotCart.some((l) => {
    ...
    const custName = snapshotCustName
    const custPhone = snapshotCustPhone
    const noteStr = snapshotNoteStr
    const order = {
      id:
    ...
    const html = buildK80ReceiptHtml(cartPrint, finalTotal, {
      ...
      invoiceNo,
      discount: disc,
      cashGiven: snapshotCashGivenNum > 0 ? snapshotCashGivenNum : finalTotal,
          ...
          <div className="pos-header-workbar">
            <div className="pos-header-workbar-inner max-md:flex max-md:flex-col max-md:gap-2 md:flex md:flex-row">
              ...
              <div className="pos-header-search-wrap max-md:order-1 max-md:basis-full search-full-mobile" ref={headerSuggestWrapRef}>
                <div className="pos-header-search-row">
                  ...
                  <input
                    ...
                    aria-controls={headerSuggestOpen ? 'pos-header-suggest-list' : undefined}
              ...
              <div className="pos-header-order-tabs-strip max-md:hidden md:block">
                ...
                {/* Nút Home chỉ hiển thị trên Mobile */}
                <button
                  className="home-btn-mobile pos-header-tab pos-header-tab--home"
