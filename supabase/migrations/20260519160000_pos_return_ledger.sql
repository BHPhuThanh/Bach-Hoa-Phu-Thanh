-- Phiếu hoàn trả POS (theo ngày thực hiện) — đồng bộ PC / mobile, báo cáo doanh thu.

CREATE TABLE IF NOT EXISTS public.pos_return_ledger (
...
CREATE INDEX IF NOT EXISTS pos_return_ledger_created_at_desc_idx ON public.pos_return_ledger (created_at DESC);
CREATE INDEX IF NOT EXISTS pos_return_ledger_order_id_idx ON public.pos_return_ledger (order_id);

COMMENT ON TABLE public.pos_return_ledger IS 'Hoàn trả POS: order_id + payload (atMs, lines, revenueSub, costSub, …).';

ALTER TABLE public.pos_return_ledger ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "pos_return_ledger_allow_all_anon" ON public.pos_return_ledger;
CREATE POLICY "pos_return_ledger_allow_all_anon"
  ON public.pos_return_ledger FOR ALL TO anon USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "pos_return_ledger_allow_all_auth" ON public.pos_return_ledger;
CREATE POLICY "pos_return_ledger_allow_all_auth"
  ON public.pos_return_ledger FOR ALL TO authenticated USING (true) WITH CHECK (true);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.pos_return_ledger TO anon, authenticated;
